...
// -------------Added by Mao yongjun 2008-02-25------- Can be delete if w32api updated
...
// Operations
	...
	{
		InsertText(TBase::GetWindowTextLength(), lpstrText, bNoScroll, bCanUndo);
	}
...
{
	typedef CTreeViewCtrlT< TBase > BaseClass;
...
// Attributes
	...
	DWORD GetTextMode() const
	{
		ATLASSERT(::IsWindow(m_hWnd));
		return (DWORD)::SendMessage(m_hWnd, EM_GETTEXTMODE, 0, 0L);
	}

	BOOL SetTextMode(DWORD enumTextMode)
	{
		ATLASSERT(::IsWindow(m_hWnd));
...
{
	typedef CListBoxT< TBase > BaseClass;

public:
	using BaseClass::m_hWnd;
	using BaseClass::GetStyle;
	using BaseClass::GetParent;
	using BaseClass::GetWndClassName;
