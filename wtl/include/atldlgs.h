...
// -----------------Add by Mao yongjun 2008.02.25 ------------ Can be delete if w32api update
...
{
	typedef ATL::CWindowImplBaseT< TBase > BaseClass;
...
// Message map and handlers
	...
	LRESULT OnCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		if(HIWORD(wParam) == BN_CLICKED && (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDCANCEL) &&
		   ((m_psh.dwFlags & PSH_MODELESS) != 0) && (BaseClass::GetActivePage() == NULL))
	...
	LRESULT OnSysCommand(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		if(((m_psh.dwFlags & PSH_MODELESS) == PSH_MODELESS) && ((wParam & 0xFFF0) == SC_CLOSE))
			BaseClass::SendMessage(WM_CLOSE);
		else
			bHandled = FALSE;
		return 0;
	}
...
{
	typedef ATL::CDialogImplBaseT< TBase > BaseClass;
...
// Callback function and overrideables
	...
	{
		UNUSED_ALWAYS(hWnd);   // avoid level 4 warning
		ATLASSERT(hWnd == NULL);
		T* pT = (T*)ppsp->lParam;
		UINT uRet = 0;
...
// Message map and handlers
	...
	LRESULT OnNotify(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
	{
		...
		case PSN_HELP:
			...
			break;
#ifndef _WIN32_WCE
#if (_WIN32_IE >= 0x0400) && defined(PSN_GETOBJECT)
		case PSN_GETOBJECT:
			if(!pT->OnGetObject((LPNMOBJECTNOTIFY)lParam))
				bHandled = FALSE;
			break;
#endif // (_WIN32_IE >= 0x0400)
#if (_WIN32_IE >= 0x0500)
    #ifndef PSN_TRANSLATEACCELERATOR
        #define PSN_TRANSLATEACCELERATOR (PSN_FIRST-12)
    #endif
    #ifndef PSN_QUERYINITIALFOCUS
        #define PSN_QUERYINITIALFOCUS   (PSN_FIRST-13)
    #endif
    #ifndef PSNRET_MESSAGEHANDLED
...
#define _ATL_NO_HOSTING   // Add by Mao yongjun 2008.02.24
...
// Constructor/destructor
	...
	~CAxPropertyPageImpl()
	{
		if(m_hInitData != NULL)
		{
			BaseClass::UnlockResource(m_hInitData);
			BaseClass::FreeResource(m_hInitData);
		}
		if(m_hDlgRes != NULL)
		{
