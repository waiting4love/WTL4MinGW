		...
		if (m_atom == 0)
		{
			...
			if(m_atom == 0)
			{
				...
				if (m_lpszOrigName != NULL)
				{
					...
					_WTL_DEFINE_STRUC(WNDCLASSEX, wc);
				...
				_WTL_DEFINE_STRUC(WNDCLASSEX, wcTemp);
...
{ \
	...
	{ \
		{ sizeof(WNDCLASSEX), 0, WindowClass::StartWindowProc, \
...
{ \
	static WTL::CFrameWndClassInfo wc = \
	{ \
		{ sizeof(WNDCLASSEX), style, WindowClass::StartWindowProc, \
...
{ \
	static CFrameWndClassInfo wc = \
	{ \
		{ style, WindowClass::StartWindowProc, \
...
{ \
	static CFrameWndClassInfo wc = \
	{ \
		{ NULL, WindowClass::StartWindowProc, \
...
{
public:
	typedef ATL::CWindowImplBaseT< TBase, TWinTraits > BaseClass;
	typedef CFrameWindowImplBase WindowClass;

	using BaseClass::m_hWnd;
	using BaseClass::m_thunk;
	using BaseClass::m_pfnSuperWindowProc;
	using BaseClass::GetClientRect;
	using BaseClass::GetStyle;
...
// Methods
			...
			DWORD dwStyle = ATL_SIMPLE_TOOLBAR_STYLE, UINT nID = ATL_IDW_TOOLBAR)
	{
        ...
        //----------------------------------fix for windres issue-----------------------  by Mao yongjun 2008.04.24
            ...
            ATLASSERT((pUnkToolbar[2]+3)*sizeof(DWORD) == ::SizeofResource(hInst, hRsrc));
...
#endif // !_WIN32_WCE
		{
			_WTL_DEFINE_STRUC(TBADDBITMAP, tbab);
			tbab.hInst = hInst;
...
public:
	...
	using BaseClass::m_pfnSuperWindowProc;
	using BaseClass::m_hAccel;
	using BaseClass::m_hWnd;
	using BaseClass::m_hWndToolBar;
#if (_WIN32_IE >= 0x0500) && !defined(_WIN32_WCE)
	typedef typename BaseClass::_ChevronMenuInfo _ChevronMenuInfo;
#endif
...
public:
	...
	using BaseClass::m_hWnd;
	using BaseClass::m_hWndToolBar;
	using BaseClass::m_hMenu;
	using BaseClass::m_hAccel;
	using BaseClass::m_pCurrentMsg;
	using BaseClass::m_hWndMDIClient;
	using BaseClass::m_hWndClient;
	using BaseClass::GetStyle;
	using BaseClass::m_pfnSuperWindowProc;
	using BaseClass::ModifyStyle;
	using BaseClass::SetMDIFrameMenu;
#if (_WIN32_IE >= 0x0500) && !defined(_WIN32_WCE)
	typedef typename BaseClass::_ChevronMenuInfo _ChevronMenuInfo;
#endif
	HWND Create(HWND hWndParent = NULL, ATL::_U_RECT rect = NULL, LPCTSTR szWindowName = NULL,
...
public:
	typedef CFrameWindowImplBase<TBase, TWinTraits > BaseClass;
	using BaseClass::m_pfnSuperWindowProc;
	using BaseClass::MDIMaximize;
	using BaseClass::MDIGetActive;
	using BaseClass::GetMDIFrame;
	using BaseClass::GetExStyle;
	using BaseClass::GetStyle;
