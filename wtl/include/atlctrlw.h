...
{
public:
    typedef ATL::CSimpleArray< T > BaseClass;

    using BaseClass::GetSize;
    using BaseClass::m_aT;
    using BaseClass::RemoveAt;
...
public:
    ...
    typedef typename BaseClass::WindowClass WindowClass;

    using BaseClass::m_hWnd;
    using TBase::m_hWndHook;
    using TBase::s_pmapMsgHook;
    using TBase::s_pCurrentBar;
    using TBase::s_hCreateHook;
    using TBase::s_bW2K;
    using TBase::s_bStaticInit;
    using TBase::m_stackMenuHandle;
    using TBase::m_stackMenuWnd;
    using BaseClass::DefWindowProc;
    using BaseClass::PostMessage;
    using BaseClass::SendMessage;
    using BaseClass::UpdateWindow;
    using BaseClass::ScreenToClient;
    using BaseClass::HitTest;
    using BaseClass::MapWindowPoints;
    using BaseClass::GetClientRect;
    using BaseClass::GetParent;

    typedef typename TBase::CMsgHookMap CMsgHookMap;
    typedef typename TBase::_MsgHookData _MsgHookData;
...
// Methods
	...
	BOOL _LoadImagesHelper(ATL::_U_STRINGorID image, bool bMapped, UINT nFlags = 0, LPCOLORMAP lpColorMap = NULL, int nMapSize = 0)
	{
        ...
        if (pData->wVersion != 1)
        {
        //---------------------------fix for windres issue------------------------------ by Mao yongjun 2008.04.24
            DWORD *pUnkToolbar = (DWORD*)pData;
            ATLASSERT((pUnkToolbar[2]+3)*sizeof(DWORD) == ::SizeofResource(hInstance, hRsrc));

            WORD wSize = pUnkToolbar[2];
            WORD* pwData = (WORD*)_alloca((4+wSize)*sizeof(WORD));
            pwData[0]=1;
            pwData[1]=pUnkToolbar[0];
            pwData[2]=pUnkToolbar[1];
            pwData[3]=wSize;
            for(WORD i=0; i<wSize;i++) pwData[i+4]=pUnkToolbar[i+3];

            pData = (_ToolBarData*)pwData;
        //------------------------------------------------------------------------------
        }
...
// public API handlers - these stay to support chevrons in atlframe.h
	...
	LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		LRESULT lRet = BaseClass::DefWindowProc(uMsg, wParam, lParam);
		// get and use system settings
		T* pT = static_cast<T*>(this);
		pT->GetSystemSettings();
		// Parent init
		ATL::CWindow wndParent = BaseClass::GetParent();
	...
	LRESULT OnChar(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		else
		{
			...
			{
#endif // (_WIN32_IE >= 0x0500)
				BaseClass::PostMessage(WM_KEYDOWN, VK_DOWN, 0L);
				if(wParam != VK_RETURN)
					SetHotItem(nBtn);
#if (_WIN32_IE >= 0x0500)
			}
	...
	LRESULT OnInitMenu(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		BaseClass::SendMessage(WM_MENUSELECT, MAKEWPARAM(nIndex, MF_POPUP|MF_HILITE), (LPARAM)m_hMenu);
		bHandled = FALSE;
		return 1;
	}
...
public:
    ...
    using TBase::m_stackMenuHandle;
    using TBase::m_stackMenuWnd;
    using BaseClass::m_bLayoutRTL;
    using BaseClass::m_dwExtendedStyle;
    using BaseClass::m_bParentActive;
    ...
    using BaseClass::UpdateWindow;
    using BaseClass::ScreenToClient;
    using BaseClass::ClientToScreen;
    using BaseClass::SetCapture;
    using BaseClass::HitTest;
    using BaseClass::MapWindowPoints;
    using BaseClass::GetWindowRect;
    using BaseClass::GetClientRect;
    using BaseClass::RedrawWindow;
    using BaseClass::GetMenu;
    using BaseClass::GetParent;
    using BaseClass::SetWindowPos;
    //typedef typename TBase::CMsgHookMap CMsgHookMap;
    //typedef typename TBase::_MsgHookData _MsgHookData;
// Data members
