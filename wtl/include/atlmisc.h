...
int const rgInitData[] = { -1, 0, 0, 0 };
CStringData* const _atltmpDataNil = (CStringData*)&rgInitData;
LPCTSTR const _atltmpPchNil = (LPCTSTR)(((BYTE*)&rgInitData) + sizeof(CStringData));
...
protected:
	...
	{
		if(AllocBeforeWrite(nSrcLen) && m_pchData != _GetEmptyString().m_pchData)  // Add by mao yongjun 2005-02-08  (&& m_pchData != _GetEmptyString().m_pchData)
		{
...
// Attributes
	ULONGLONG GetFileSize() const
	{
		...
		_WTL_DEFINE_STRUC(ULARGE_INTEGER, nFileSize);
...
{
#ifdef _DEBUG
	WORD wID = (WORD)(UINT)cursor.m_lpstr;
...
{
#ifdef _DEBUG
	WORD wID = (WORD)(UINT)icon.m_lpstr;
