...
public:
	...
	void SetCaptionFont()
	{
		_WTL_DEFINE_STRUC(NONCLIENTMETRICS, ncm);
...
#if !defined(_ATL_NO_MSIMG) && !defined(_WIN32_WCE) && (WINVER >= 0x0500)
...
// Text Functions
	...
	int GetTextFace(CStringT& strFace) const
#endif
	{
		ATLASSERT(m_hDC != NULL);
...
struct DIBINFO16 : public BITMAPINFO
{
	...
	DIBINFO16(SIZE size)
	{
		BITMAPINFOHEADER bmih = { sizeof(BITMAPINFOHEADER), size.cx, size.cy,
		                          1, 16, BI_BITFIELDS, 2 * size.cx * size.cy , 0, 0, 3, 0 };
...
{
	_WTL_DEFINE_STRUC(DIBSECTION, ds);
	LPBYTE pDib = NULL;
	bool bCopied = false;
