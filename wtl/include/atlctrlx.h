...
// Message map and handlers
	...
	LRESULT OnLButtonDown(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		if((m_dwExtendedStyle & BMPBTN_AUTOFIRE) != 0)
		{
			...
			CButton::SetTimer(ID_TIMER_FIRST, nElapse);
		}
		return lRet;
	}
	...
	LRESULT OnTimer(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		{
		case ID_TIMER_FIRST:
			CButton::KillTimer(ID_TIMER_FIRST);
			if(m_fPressed == 1)
			{
				::SendMessage(GetParent(), WM_COMMAND, MAKEWPARAM(GetDlgCtrlID(), BN_CLICKED), (LPARAM)m_hWnd);
				int nElapse = 250;
				...
				CButton::SetTimer(ID_TIMER_REPEAT, nElapse);
			}
			break;
		case ID_TIMER_REPEAT:
			if(m_fPressed == 1)
				...
				CButton::KillTimer(ID_TIMER_REPEAT);
			break;
...
{
public:
    typedef CCheckListViewCtrlImpl WindowClass;
    typedef ATL::CWindowImpl< T, TBase, TWinTraits> BaseClass;
...
public:
    ...
    using BaseClass::m_hWnd;
    using BaseClass::SetWindowText;
    using BaseClass::GetClientRect;
    using BaseClass::GetDlgCtrlID;
    using BaseClass::GetParent;
    using BaseClass::InvalidateRect;
    using BaseClass::UpdateWindow;
    using BaseClass::SetFocus;
    using BaseClass::SetCapture;
    using BaseClass::ScreenToClient;
    using BaseClass::GetWindowText;
...
{
public:
    typedef CMultiPaneStatusBarCtrlImpl WindowClass;
    typedef ATL::CWindowImpl< T, TBase > BaseClass;
...
// Methods
	...
	{
//#if (_MSC_VER >= 1300)
		return ATL::CWindowImpl< T, TBase >::Create(hWndParent, CWindow::rcDefault, lpstrText, dwStyle, 0, nID);
//#else // !(_MSC_VER >= 1300)
...
{
public:
    typedef CPaneContainerImpl WindowClass;
    typedef ATL::CWindowImpl< T, TBase, TWinTraits > BaseClass;
...
// Methods
			...
			DWORD dwExStyle = 0, UINT nID = 0, LPVOID lpCreateParam = NULL)
	{
		...
		return ATL::CWindowImpl< T, TBase, TWinTraits >::Create(hWndParent, CWindow::rcDefault, NULL, dwStyle, dwExStyle, nID, lpCreateParam);
	}
...
// Implementation - overrideable methods
	...
	void CreateCloseButton()
	{
		...
		m_tb.Create(m_hWnd, CWindow::rcDefault, NULL, WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | CCS_NODIVIDER | CCS_NORESIZE | CCS_NOPARENTALIGN | CCS_NOMOVEY | TBSTYLE_TOOLTIPS | TBSTYLE_FLAT, 0);
...
{
public:
    typedef CSortListViewCtrlImpl WindowClass;
