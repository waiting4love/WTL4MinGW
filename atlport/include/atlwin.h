...
struct _ATL_WNDCLASSINFOA
{
	...
	{
		return _ATLAPI_NS AtlModuleRegisterWndClassInfoA(&_Module, this, p);
	}
};
struct _ATL_WNDCLASSINFOW
{
	...
	ATOM Register(WNDPROC* p)
	{
		return _ATLAPI_NS AtlModuleRegisterWndClassInfoW(&_Module, this, p);
	}
};
...
{ \
	...
	{ \
		{ sizeof(WNDCLASSEX), CS_HREDRAW | CS_VREDRAW | CS_DBLCLKS, WindowClass::StartWindowProc, \
...
{
	typedef CWindowImplRoot<TBase> BaseClass;
...
{
	typedef CWindowImplBaseT<TBase, TWinTraits> BaseClass;

public:
	typedef T WindowClass;
			...
			UINT nID = 0, LPVOID lpCreateParam = NULL)
	{
		if (T::GetWndClassInfo().m_lpszOrigName == NULL)
			T::GetWndClassInfo().m_lpszOrigName = BaseClass::GetWndClassName();
		ATOM atom = T::GetWndClassInfo().Register(&m_pfnSuperWindowProc);

		dwStyle = T::GetWndStyle(dwStyle);
		dwExStyle = T::GetWndExStyle(dwExStyle);

		return BaseClass::Create(hWndParent, rcPos, szWindowName,
...
{
	typedef CDialogImplBaseT< TBase > BaseClass;
public:
	using BaseClass::m_hWnd;
	using BaseClass::m_thunk;
#ifdef _DEBUG
